...
* _potatoes_: Impact of storage period and cooking on potatoes' flavor
...
```{r, eval = FALSE}
potatoes <- read.csv(file = "data/potatoes.csv", sep = ",", header = TRUE)
```
...
```{r}
potatoes <- read_tsv("data/potatoes.txt", col_types = "cccccccc", col_names = properties)
```
...
```{r, eval = FALSE}
# Only two columns are specified in the header “a” and “b”
read_csv("a,b\n1,2,3\n4,5,6")
# There are only two values in row 1, so column c is set to NA
read_csv("a,b,c\n1,2\n1,2,3,4")
# The values are separated by “;” not “,”. Use read_csv2 instead
read_csv("a;b\n1;3")
```
...
pop <- read_excel("data/urbanpop.xlsx", sheet=2,
