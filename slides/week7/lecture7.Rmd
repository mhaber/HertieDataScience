...
library(readxl)
library(tidyverse)
iiag <- read_xls("data/iiag.xls", sheet = 5, skip = 5) 
iiag %>% 
...
c. Multiple values are stored in one column

Which of the following is a principle of tidy data?

a. Each observation forms a row, each variable forms a column, and each type of observational unit forms a table

b. Each value belongs to a variable and an observation

c. A dataset is a collection of values
...
1. Prepare your data set for analysis \alert{25%}

    + Load the data and perform the necessary data cleaning and manipulation steps
    
2. Perform exploratory data analysis and visualize interesting descriptive facts about your data \alert{25%}

3. Develop and specify ideas for analyzing your data and run the appropriate statistical model \alert{25%} 

    + e.g. regression analysis, machine learning, etc.

4. Present and discuss your results \alert{25%}
...
```{r, message = FALSE, warning = FALSE}
url <- paste0("https://raw.githubusercontent.com/mhaber/HertieDataScience/",
              "master/slides/week7/data/")
films <- read_csv(paste0(url, "films.csv"))
people <- read_csv(paste0(url, "people.csv"), col_types = "iccc")
...
Set up the database
```{r}
con <- dbConnect(SQLite(), dbname = "data/films.sqlite")
```
...
```{r}
DBI::dbGetQuery(con, "SELECT AVG(duration / 60.0) AS avg_duration_hours
                FROM films")
```
...
final task
```{r, eval = FALSE}
DBI::dbGetQuery(con, "SELECT country, AVG(budget) AS avg_budget, 
